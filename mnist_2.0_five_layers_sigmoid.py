...
Y1 = tf.nn.sigmoid(x=(tf.matmul(XX, W1) + B1))
Y2 = tf.nn.sigmoid(x=(tf.matmul(Y1, W2) + B2))
Y3 = tf.nn.sigmoid(x=(tf.matmul(Y2, W3) + B3))
Y4 = tf.nn.sigmoid(x=(tf.matmul(Y3, W4) + B4))
...
learning_rate = 0.003
train_step = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss=cross_entropy)
