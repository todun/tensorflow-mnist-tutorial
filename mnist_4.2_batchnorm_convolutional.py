...
pkeep_conv = tf.placeholder(dtype=tf.float32)
...
def compatible_convolutional_noise_shape(Y):
    noiseshape = tf.shape(input=Y)
    noiseshape = noiseshape * tf.constant(value=[1, 0, 0, 1]) + tf.constant(value=[0, 1, 1, 0])
    return noiseshape
...
W1 = tf.Variable(
    initial_value=tf.truncated_normal(shape=[6, 6, 1, K], stddev=0.1))  # 6x6 patch, 1 input channel, K output channels
B1 = tf.Variable(initial_value=tf.constant(value=0.1, dtype=tf.float32, shape=[K]))
W2 = tf.Variable(initial_value=tf.truncated_normal(shape=[5, 5, K, L], stddev=0.1))
B2 = tf.Variable(initial_value=tf.constant(value=0.1, dtype=tf.float32, shape=[L]))
W3 = tf.Variable(initial_value=tf.truncated_normal(shape=[4, 4, L, M], stddev=0.1))
B3 = tf.Variable(initial_value=tf.constant(value=0.1, dtype=tf.float32, shape=[M]))

W4 = tf.Variable(initial_value=tf.truncated_normal(shape=[7 * 7 * M, N], stddev=0.1))
B4 = tf.Variable(initial_value=tf.constant(value=0.1, dtype=tf.float32, shape=[N]))
W5 = tf.Variable(initial_value=tf.truncated_normal(shape=[N, 10], stddev=0.1))
B5 = tf.Variable(initial_value=tf.constant(value=0.1, dtype=tf.float32, shape=[10]))

# The model
# batch norm scaling is not useful with relus
# batch norm offsets are used instead of biases
stride = 1  # output is 28x28
Y1l = tf.nn.conv2d(input=X, filter=W1, strides=[1, stride, stride, 1], padding='SAME')
Y1bn, update_ema1 = batchnorm(Ylogits=Y1l, is_test=tst, iteration=iter, offset=B1, convolutional=True)
Y1r = tf.nn.relu(features=Y1bn)
Y1 = tf.nn.dropout(x=Y1r, keep_prob=pkeep_conv, noise_shape=compatible_convolutional_noise_shape(Y=Y1r))
stride = 2  # output is 14x14
Y2l = tf.nn.conv2d(input=Y1, filter=W2, strides=[1, stride, stride, 1], padding='SAME')
Y2bn, update_ema2 = batchnorm(Ylogits=Y2l, is_test=tst, iteration=iter, offset=B2, convolutional=True)
Y2r = tf.nn.relu(features=Y2bn)
Y2 = tf.nn.dropout(x=Y2r, keep_prob=pkeep_conv, noise_shape=compatible_convolutional_noise_shape(Y=Y2r))
stride = 2  # output is 7x7
Y3l = tf.nn.conv2d(input=Y2, filter=W3, strides=[1, stride, stride, 1], padding='SAME')
Y3bn, update_ema3 = batchnorm(Ylogits=Y3l, is_test=tst, iteration=iter, offset=B3, convolutional=True)
Y3r = tf.nn.relu(features=Y3bn)
Y3 = tf.nn.dropout(x=Y3r, keep_prob=pkeep_conv, noise_shape=compatible_convolutional_noise_shape(Y=Y3r))

# reshape the output from the third convolution for the fully connected layer
YY = tf.reshape(tensor=Y3, shape=[-1, 7 * 7 * M])

Y4l = tf.matmul(a=YY, b=W4)
Y4bn, update_ema4 = batchnorm(Ylogits=Y4l, is_test=tst, iteration=iter, offset=B4)
Y4r = tf.nn.relu(features=Y4bn)
Y4 = tf.nn.dropout(x=Y4r, keep_prob=pkeep)
...
allbiases = tf.concat(
    values=[tf.reshape(tensor=B1, shape=[-1]), tf.reshape(tensor=B2, shape=[-1]), tf.reshape(tensor=B3, shape=[-1]),
            tf.reshape(tensor=B4, shape=[-1]), tf.reshape(tensor=B5, shape=[-1])], axis=0)
conv_activations = tf.concat(values=[tf.reshape(tensor=tf.reduce_max(input_tensor=Y1r, axis=[0]), shape=[-1]),
                                     tf.reshape(tensor=tf.reduce_max(input_tensor=Y2r, axis=[0]), shape=[-1]),
                                     tf.reshape(tensor=tf.reduce_max(input_tensor=Y3r, axis=[0]), shape=[-1])], axis=0)
dense_activations = tf.reduce_max(input_tensor=Y4r, axis=[0])
...
datavis = tensorflowvisu.MnistDataVis(title4="batch-max conv activation", title5="batch-max dense activations",
...
def training_step(i, update_test_data, update_train_data):
    ...
    if update_train_data:
        a, c, im, ca, da = sess.run(fetches=[accuracy, cross_entropy, I, conv_activations, dense_activations],
        ...
        datavis.append_data_histograms(x=i, datavect1=ca, datavect2=da)

    # compute test values for visualisation
    if update_test_data:
        a, c, im = sess.run(fetches=[accuracy, cross_entropy, It],
                            feed_dict={X: mnist.test.images, Y_: mnist.test.labels, tst: True, pkeep: 1.0,
                                       pkeep_conv: 1.0})
    ...
    sess.run(fetches=train_step,
             feed_dict={X: batch_X, Y_: batch_Y, lr: learning_rate, tst: False, pkeep: 0.75, pkeep_conv: 1.0})
    sess.run(fetches=update_ema, feed_dict={X: batch_X, Y_: batch_Y, tst: False, iter: i, pkeep: 1.0, pkeep_conv: 1.0})


datavis.animate(compute_step=training_step, iterations=10001, train_data_update_freq=20, test_data_update_freq=100)
