...
W4 = tf.Variable(initial_value=tf.truncated_normal(shape=[N, O], stddev=0.1))
B4 = tf.Variable(initial_value=tf.ones(shape=[O]) / 10)
W5 = tf.Variable(initial_value=tf.truncated_normal(shape=[O, 10], stddev=0.1))
B5 = tf.Variable(initial_value=tf.zeros(shape=[10]))

# The model, with dropout at each layer
XX = tf.reshape(tensor=X, shape=[-1, 28 * 28])

Y1 = tf.nn.relu(features=tf.matmul(a=XX, b=W1) + B1)
Y1d = tf.nn.dropout(x=Y1, keep_prob=pkeep)

Y2 = tf.nn.relu(features=tf.matmul(a=Y1d, b=W2) + B2)
Y2d = tf.nn.dropout(x=Y2, keep_prob=pkeep)

Y3 = tf.nn.relu(features=tf.matmul(a=Y2d, b=W3) + B3)
Y3d = tf.nn.dropout(x=Y3, keep_prob=pkeep)

Y4 = tf.nn.relu(features=tf.matmul(a=Y3d, b=W4) + B4)
Y4d = tf.nn.dropout(x=Y4, keep_prob=pkeep)

Ylogits = tf.matmul(a=Y4d, b=W5) + B5
...
def training_step(i, update_test_data, update_train_data):
    ...
    sess.run(fetches=train_step, feed_dict={X: batch_X, Y_: batch_Y, pkeep: 0.75, lr: learning_rate})
